...
const Chat: React.FC<ChatProps> = ({ 
    ...
    // Rola para o fim sempre que as mensagens mudarem
    useEffect(() => {
    ...
    // Busca a lista de contatos/conversas
    ...
    const fetchMessages = useCallback(async (chatId: string, isSilent = false) => {
        ...
        try {
            ...
            const sortedMessages = [...rawMessages].sort((a, b) => {
                const timeA = a.timestamp || a.messageTimestamp || (a.created_at ? new Date(a.created_at).getTime() / 1000 : 0);
                const timeB = b.timestamp || b.messageTimestamp || (b.created_at ? new Date(b.created_at).getTime() / 1000 : 0);
                return timeA - timeB; 
            });
            ...
            const mappedMessages: ChatMessage[] = sortedMessages.map(msg => {
                ...
                if (content) {
                    if (content.imageMessage) {
                        ...
                        const fileName = content.documentMessage.title || content.documentMessage.fileName || 'Arquivo';
                        text = `${fileName}${content.documentMessage.caption ? '\n\n' + content.documentMessage.caption : ''}`;
                    } else if (content.contactMessage) {
                        text = `👤 Contato: ${content.contactMessage.displayName || 'Sem nome'}`;
                ...
                if (timestamp) {
                    ...
                    timeStr = new Date().toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' });
                }

                return {
                    id: msg.id || msg.key?.id || Math.random().toString(),
                    sender: isFromMe ? 'admin' : 'user',
                    text: text || (mediaUrl ? '' : '[Tipo de mensagem não suportado]'),
                    timestamp: timeStr,
                    mediaUrl,
                    mediaType
                };
    ...
    // Polling das mensagens do chat aberto
    useEffect(() => {
    ...
    const handleSendMessage = async (e?: React.FormEvent) => {
        ...
        // Se houver áudio gravado e preview ativo
        ...
        <div className="flex-1 flex items-center justify-center text-gray-500">
            <div className="text-center p-8">
                ...
                <h2 className="text-xl font-bold text-gray-800 dark:text-white">
                    {whatsappStatus !== 'Conectado' ? 'WhatsApp Desconectado' : 'Selecione uma conversa'}
                </h2>
                <p className="mt-2 text-gray-600 dark:text-gray-400 max-w-xs mx-auto">
                    {whatsappStatus !== 'Conectado' 
                        ? 'Conecte sua instância nas configurações para usar o chat.' 
                                   ...
                                   {messages.map((msg, idx) => {
                                       ...
                                       return (
                                           <div key={msg.id || idx} className={`flex ${msg.sender === 'admin' ? 'justify-end' : 'justify-start'}`}>
                                               <div className={`max-w-[75%] p-3 rounded-2xl shadow-sm text-sm ${msg.sender === 'admin' ? 'bg-primary text-white rounded-tr-none' : isSticker ? '' : 'bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-tl-none border border-gray-100 dark:border-gray-700'}`}>
                                                   
                                                   {/* Mídia: Imagem/Figurinha */}
                                                   {msg.mediaUrl && (msg.mediaType === 'image' || msg.mediaType === 'sticker') && (
                                                       <div className={isSticker ? "mb-1" : "mb-2 -mx-1 -mt-1"}>
                                                           <img src={msg.mediaUrl} alt="Media" className={`rounded-lg h-auto object-cover cursor-pointer hover:opacity-95 ${isSticker ? 'w-32' : 'max-w-full'}`} onClick={() => window.open(msg.mediaUrl, '_blank')}/>
                                                       </div>
                                                   )}
                                                   ...
                                                   {msg.mediaUrl && msg.mediaType === 'video' && (
                                                       <div className="mb-2 -mx-1 -mt-1">
                                                           <video src={msg.mediaUrl} controls className="rounded-lg max-w-full h-auto" />
                                                       </div>
                                                   )}
                                                   ...
                                                   {msg.mediaUrl && msg.mediaType === 'document' && (
                                                        <div className="mb-2 flex items-center gap-3 p-3 bg-gray-100 dark:bg-gray-700/50 rounded-xl border border-gray-200 dark:border-gray-600">
                                                            ...
                                                            <button onClick={() => window.open(msg.mediaUrl, '_blank')} className="text-left flex-1 hover:underline truncate">
                                                                <span className="font-bold block truncate">Abrir Documento</span>
                                                                <span className="text-[10px] opacity-60 uppercase font-bold">PDF / Office</span>
                                                            </button>
                                                        </div>
                                                   )}
                                                   ...
                                                   <span className={`text-[9px] mt-1 block text-right opacity-60 font-bold uppercase ${isSticker ? 'text-gray-400' : ''}`}>{msg.timestamp}</span>
                                               </div>
                                           </div>
                                       );
                                   })}
                            ...
                            {isRecording ? (
                                <div className="flex items-center justify-between bg-red-50 dark:bg-red-900/20 p-2 rounded-xl border border-red-100 dark:border-red-900/30 animate-pulse">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-sm font-bold text-red-600 dark:text-red-400">Gravando: {formatTime(recordingTime)}</span>
                                    </div>
                                    <div className="flex gap-2">
                                        <button onClick={cancelRecording} className="p-2 text-gray-500 hover:text-red-600 transition-colors" title="Cancelar"><TrashIcon className="w-5 h-5"/></button>
                                        <button onClick={stopRecording} className="p-2 bg-red-600 text-white rounded-lg shadow-md hover:bg-red-700 transition-colors" title="Parar e Visualizar"><StopIcon className="w-5 h-5"/></button>
                                    </div>
                                </div>
                            ) : recordedAudioUrl ? (
                                <div className="flex flex-col gap-3 bg-gray-50 dark:bg-gray-700/50 p-3 rounded-xl border border-gray-100 dark:border-gray-700">
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="text-[10px] font-bold text-gray-500 uppercase">Visualizar Áudio Gravado</span>
                                        <button onClick={cancelRecording} className="text-gray-400 hover:text-red-500"><TrashIcon className="w-4 h-4"/></button>
                                    </div>
                                    <audio src={recordedAudioUrl} controls className="w-full h-10" />
                                    <div className="flex justify-end">
                                        <button onClick={() => handleSendMessage()} className="bg-primary hover:bg-primary-dark text-white font-bold py-2 px-6 rounded-lg text-sm transition-all shadow-lg active:scale-95">Enviar Áudio</button>
                                    </div>
                                </div>
                            ) : (
                                <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                                    <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Digite uma mensagem..." className="flex-1 bg-gray-100 dark:bg-gray-700 rounded-xl py-3 px-4 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary transition-all" disabled={whatsappStatus !== 'Conectado'} />
                                    <button type="button" onClick={startRecording} className="text-gray-500 hover:text-primary dark:hover:text-white p-3 transition-colors" disabled={whatsappStatus !== 'Conectado'} title="Gravar Áudio"><MicrophoneIcon className="w-6 h-6" /></button>
                                    <button type="submit" className="bg-primary hover:bg-primary-dark text-white rounded-xl p-3 shadow-lg shadow-primary/20 transition-all active:scale-95 disabled:bg-gray-400 disabled:shadow-none" disabled={whatsappStatus !== 'Conectado' || !newMessage.trim()}>
